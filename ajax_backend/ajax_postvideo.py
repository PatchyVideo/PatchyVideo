...
if USE_RQ :
	...
	def ajax_postvideo_batch_do(rd, user, data):
		...
		for idx, (url, (obj, cleanURL)) in enumerate(zip(data.videos, cleanURL_objs)) :
			...
			task = q.enqueue(postVideo, cleanURL, data.tags, obj, dst_copy, dst_playlist, dst_rank + next_idx, unique_ids if as_copies else [], user)
		if succeed :
			ret = makeResponseSuccess({
				"task_id": task.get_id()
			})
	...
	def ajax_postvideo_batch_do(rd, user, data):
		...
		for idx, (url, (obj, cleanURL)) in enumerate(zip(data.videos, cleanURL_objs)) :
			...
			if obj is None:
				succeed = False
			next_idx = idx if dst_rank >= 0 else 0
			result_msg, result_id = postVideo(cleanURL, data.tags, obj, dst_copy, dst_playlist, dst_rank + next_idx,  unique_ids if as_copies else [], user)
