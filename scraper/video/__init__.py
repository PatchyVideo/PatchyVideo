			...
			async with aiohttp.ClientSession() as session:
				...
				async with session.get(link, headers = self.HEADERS_NO_UTF8, cookies = cookie) as resp:
					if resp.status == 200 :
						...
						return makeResponseFailed({'status_code': resp.status, 'resp': await resp.text(), 'link': link})
		except Exception as ex :
			import traceback
...
_crawler_modules = [ 'Bilibili', 'Youtube', 'Nicovideo', 'Twitter', 'Acfun', 'IPFS', 'Zcool', 'SinaMobile', 'Xigua', 'BilibiliAudio' ]
