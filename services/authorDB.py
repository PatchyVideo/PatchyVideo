...
def createOrModifyAuthorRecord(user, author_type, tagid, common_tags, user_spaces, desc, avatar_file_key = None, pv_user_id: Optional[ObjectId] = None) :
	...
	with MongoTransaction(client) as s :
		...
		if pv_user_id is not None :
			pv_user_id = ObjectId(pv_user_id)
		if existing_record :
...
def associateWithPvUser(user, tagid: int, pv_uid: ObjectId) :
	filterOperation('associateWithPvUser', user)
	user_obj = db.users.find_one({'_id': ObjectId(pv_uid)})
	if user_obj is None :
	...
	author_obj = db.authors.find_one({'_id': tag_obj['author']})
	assert author_obj
	if 'pv_user_id' in author_obj or 'linked_tagid' in user_obj :
		raise UserError('ASSOCIATION_ALREADY_EXIST')
	author_obj['pv_user_id'] = pv_uid
	user_obj['linked_tagid'] = tag_obj['id']
	db.authors.save(author_obj)
	db.users.save(user_obj)

def disassociateWithPvUser(user, tagid: int, pv_uid: ObjectId) :
	filterOperation('disassociateWithPvUser', user)
	user_obj = db.users.find_one({'_id': ObjectId(pv_uid)})
	if user_obj is None :
		raise UserError('USER_NOT_EXIST')
	tag_obj = tagdb._tag(tagid)
	...
	author_obj = db.authors.find_one({'_id': tag_obj['author']})
	assert author_obj
	if 'pv_user_id' in author_obj and 'linked_tagid' in user_obj and author_obj['pv_user_id'] == pv_uid and user_obj['linked_tagid'] == tag_obj['id'] :
		del author_obj['pv_user_id']
		del user_obj['linked_tagid']
		db.authors.save(author_obj)
		db.users.save(user_obj)
	else :
		raise UserError('RECORD_NOT_FOUND')

def findTagByUser(pv_uid: ObjectId) :
	user_obj = db.users.find_one({'_id': ObjectId(pv_uid)})
	if user_obj is None :
		raise UserError('USER_NOT_EXIST')
	if 'linked_tagid' in user_obj :
		tag_obj = tagdb._tag(user_obj['linked_tagid'])
		return tag_obj
	return None

def tagRemoval(tag_obj: dict, session) :
	if 'author' in tag_obj :
		author_obj = db.authors.find_one({'_id': tag_obj['author']}, session = session)
		assert author_obj
		if 'pv_user_id' in author_obj :
			user_obj = db.users.find_one({'_id': ObjectId(author_obj['pv_user_id'])}, session = session)
			if user_obj is not None :
				del user_obj['linked_tagid']
				db.users.save(user_obj, session = session)
		# TODO: here we leave an authorDB entry dangling, we want to remove it as well
