...
class RedisLockAsync(object):
	"""
	A RedisLockAsync context manager implemented via redis SETNX/BLPOP.
	"""
	...
	def __init__(self, redis_client, name, expire=None, id=None, auto_renewal=False, strict=True):
		...
		:param id:
			...
			won't be possible.
		:param auto_renewal:
			If set to ``True``, RedisLockAsync will automatically renew the lock so that it
			...
			time, subclass RedisLockAsync, call ``super().__init__()`` then set
	...
	def extend(self, expire=None):
		...
		if error == 1:
			raise NotAcquired("RedisLockAsync %s is not acquired or it already expired." % self._name)
		elif error == 2:
			raise NotExpirable("RedisLockAsync %s has no assigned expiration time" %
							   self._name)
		elif error:
	...
	def _start_lock_renewer(self):
		...
		if self._lock_renewal_thread is not None:
			raise AlreadyStarted("RedisLockAsync refresh thread already started")
	...
	def _stop_lock_renewer(self):
		...
		self._lock_renewal_thread = None
		logger.debug("RedisLockAsync refresher has stopped")
	...
	def release(self):
		...
		.. note::
			...
			* Use ``RedisLockAsync("name", id=id_from_other_place).release()``
			* Use ``RedisLockAsync("name").reset()``
		"""
		if self._lock_renewal_thread is not None:
